...
def chunk_text(text, limit=TELEGRAM_MESSAGE_LIMIT):
    """Yield successive chunks from text of size limit."""
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        f"""Welcome to the AI Forwarder Bot! I can help you chat with an AI model
...
**Chatting with AI:**
Simply send me any message, and I'll pass it to the AI. I'll send you back the AI's response.
...
/forward - Forward the last AI response to your email. If long, it's sent as an attachment.
/set_model <model_name> - Choose an AI model. Available: {available_models_text}
  Example: `/set_model {config.AVAILABLE_GEMINI_MODELS[0] if config.AVAILABLE_GEMINI_MODELS else 'gemini-pro'}`

**Note:** Your email and model preferences are currently remembered per session. They will be persisted properly soon!"""
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if ai_response and not ai_response.startswith("Error:"):
        ...
        logger.info(f"Stored AI response for chat_id {chat_id}. Data saved.")
    else:
        logger.error(
            f"AI response was empty or an error for chat_id {chat_id}. Not storing."
        )
        # If it's an error, ai_response already contains the error message to be sent to the user

    if len(ai_response) > 9000:
        # Send as text file attachment
        file_like = io.BytesIO(ai_response.encode("utf-8"))
        file_like.name = "ai_response.txt"
        await update.message.reply_document(
            document=file_like,
            filename="ai_response.txt",
            caption="AI response is too long, sent as attachment.",
        )
    else:
...
async def forward_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if word_count > max_words_inline:
        ...
        try:
            ...
            email_body = (
                body_intro
                + "The full response is attached as a text file due to its length."
            )
...
async def error_handler(update, context):
    ...
    if update and hasattr(update, "effective_message") and update.effective_message:
        try:
            ...
            pass  # Avoid crashing if we can't send the error message
